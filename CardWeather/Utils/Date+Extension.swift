...
private extension WeekDay {
    var intoChinese: String {
        let baseString = "星期"
        return baseString + rawValue.unitTranslation()
    }
}
...
extension Date {
    ...
    static func weekDayInChinese() -> String {
        let weekday = DateClass.currentWeekDay()
        guard let enumWeekDay = WeekDay(rawValue: weekday) else {
            return "无"
        }
        return enumWeekDay.intoChinese
    }
    //MARK: - 日期的一些比较
    //是否是今天
    func isToday()->Bool {
...
extension String {
   static var simpleDateFormatter: String { return "yyyy-MM-dd" }
